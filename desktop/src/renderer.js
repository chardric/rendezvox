...
async function doSearch() {
  ...
  try {
    let url = `${BASE_URL}/api/search-song?title=${encodeURIComponent(title)}`;
...
$('modal-submit').addEventListener('click', async () => {
  const title = $('req-title').value.trim();
  if (!title) {
    showStatus('Enter a song title', false);
    return;
  }
  ...
  const body = {
    title,
    artist:        $('req-artist').value.trim()  || undefined,
    listener_name: $('req-name').value.trim()    || undefined,
    message:       $('req-message').value.trim() || undefined
  };
